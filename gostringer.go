...
// Copyright 2015 The GoStringer Authors. All rights reserved.
...
// generate methods for multiple types. The default output file is t_gostring.go,
...
var (
	...
	output    = flag.String("output", "", "output file name; default srcdir/<type>_gostring.go")
)
...
func main() {
	...
	outputName := *output
	if outputName == "" {
		baseName := fmt.Sprintf("%s_gostring.go", types[0])
...
func (g *Generator) createIndexAndNameDecl(run []Value, typeName string, suffix string) (string, string) {
	...
	nameConst := fmt.Sprintf("_%s_name%s_gostringer = %q", typeName, suffix, b.String())
	nameLen := b.Len()
	b.Reset()
	fmt.Fprintf(b, "_%s_index%s_gostringer = [...]uint%d{0, ", typeName, suffix, usize(nameLen))
...
func (g *Generator) declareNameVars(runs [][]Value, typeName string, suffix string) {
	g.Printf("const _%s_name%s_gostringer = \"", typeName, suffix)
...
func (g *Generator) buildMap(runs [][]Value, typeName string) {
	...
	g.Printf("\nvar _%s_map_gostringer = map[%s]string{\n", typeName, typeName)
	n := 0
	for _, values := range runs {
		for _, value := range values {
			g.Printf("\t%s: _%s_name_gostringer[%d:%d],\n", &value, typeName, n, n+len(value.name))
			n += len(value.name)
		}
	}
	g.Printf("}\n\n")
